...
For example, if a Process has an IOEvent with a time value of 10, then that
means that when the process hits it's 10th time step (i.e.,
Process::processorTime = 10) it should enter a blocked state for
IOEvent::duration time steps.
...
struct IOEvent {
  ...
  IOEvent(const int &t, const int &d, const unsigned int &newId)
      : id(newId), time(t), duration(d) {}

  unsigned int id;

  long time; // The time the event occurs during the process execution
  long
      duration; // The duration that the process will be Blocked by this IOEvent
};
...
}; // Used to track the process states

struct Process {
  Process()
      : id(999999), arrivalTime(-1), doneTime(-1), reqProcessorTime(0),
        processorTime(0), state(newArrival) {}

  unsigned int
      id; // The process ID, assigned when the process is admitted to the system

  long arrivalTime; // When the process will start/become runnable
  long doneTime;    // Convenience variable, use this to keep track of when a
                    // process completes
  ...
  list<IOEvent> ioEvents; // The IO events for this process, stored in order of
                          // the time into the process execution that they start
