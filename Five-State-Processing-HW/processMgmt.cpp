...
void ProcessManagement::readProcessFile(const string &fname) {
  ...
  if (!in.good()) {
    cerr << "initProcessSetFromFile error     unable to open file \"" << fname
         << "\"" << endl;
    return;
  }
...
void ProcessManagement::activateProcesses(const int &time) {
  if (m_pending.size() > 0) {
    while (m_pending.back().arrivalTime == time) {
      ...
      // m_procList.back().setState(blocked);
      // /////////////////////////////////////new code. Should have if statement
      m_pending.pop_back();
      // m_procList.back().printProcess();
    }
  }
}